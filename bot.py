...
async def cmd_start(message: Message):
    ...
    if text.startswith("/start") and " " in text:
        ...
        if args.startswith("fastconnect_"):
    ...
    text = (
        ...
        "между компаниями и специалистами.\n\n"
        "Он помогает компаниям принимать взвешенные решения при найме,\n"
        "а специалистам — подтверждать свой опыт работы.\n\n"
        "Как это работает:\n"
        "• Специалист регистрируется и привязывается к компании.\n"
        "• Компания подтверждает факт сотрудничества и данные специалиста.\n"
        "• После завершения работы компания может оставить служебную пометку.\n"
        "• Заинтересованные стороны могут проверить статус специалиста по ID.\n\n"
